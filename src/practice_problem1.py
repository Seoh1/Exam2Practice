...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        string1 = ''
        string2 = ''
        sum = self.volume - len(self.contents)
        if sum < len(additional_contents):
            for k in range(sum):
                string1 += additional_contents[k]
            self.contents += string1
            for k in range(sum, len(additional_contents)):
                string2 += additional_contents[k]
            return string2
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents) > new_volume:
            ...
            for k in range(new_volume):
                string1 = string1 + self.contents[k]
            self.contents = string1
        return string2
    ...
    def double_then_shrink(self, new_volume):
        ...
        double_content = 2 * len(self.contents)
        self.double()
        self.shrink(new_volume)
        shrink_content = len(self.contents)
        return double_content - shrink_content
    ...
    def reset(self):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        self.list.append(self.contents)
